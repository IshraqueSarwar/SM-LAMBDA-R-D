...
public class CameraController : MonoBehaviour
{
                ...
                if (Physics.Raycast(selectionRay, out RaycastHit hitObj))
                {
                    ...
                    if(Object.tag == "Selectable")
                    {
                        ...
                        if(containerName != currentObjectName && pressTime >= selectionTimer && !stillFingerDown)//<-- we select an object not selected before after a 'long' press and make sure its not from the deselection touch
                        {
                            ...
                            // if selected object is a ARDUINO computer we show the OPEN EDITOR BUTTON
                            if (currentObjectName.Contains("ArduinoComputer"))
                            {
                        ...
                        {
                            // if selected object is a ARDUINO computer we HIDE the OPEN EDITOR BUTTON
                            if (currentObjectName.Contains("ArduinoComputer"))
                            {
                            ...
                            // ??NOTE?? if the deselected object is 
                        }
    ...
    private void focusOnHitObject(RaycastHit hitObj)
    {
    ...
    private void selectAndOutlineObject(GameObject Object)
    {
    ...
    // We call this function from the show editor button
    public void makeCodeEditorVisible()
    {
        if (selectedObject != null)
        {
            Debug.Log(selectedObject.name);
            selectedObject.transform.parent.Find("Computer").Find("Canvas").gameObject.SetActive(true);
        }
